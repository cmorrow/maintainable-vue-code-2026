...
import { useUser } from "@/features/composables/composables/useUser";
import { useUserPosts } from "@/features/composables/composables/useUserPosts";
import { useUserSettings } from "@/features/composables/composables/useUserSettings";
import { useModal } from "@/features/composables/composables/useModal";
...
const { settings, toggleNotifications } = useUserSettings(userId);
  ...
  <div class="demo-page">
    <header class="page-header">
      <div class="page-header-top">
        <VSubHeading>Composables - Page After Explained</VSubHeading>
      ...
      <nav class="page-nav" aria-label="Composables pages">
        ...
        <RouterLink to="/composables/page-refactor/page-after"
          >After</RouterLink
        >
        <RouterLink to="/composables/page-refactor/page-before-explained"
          >Before Explained</RouterLink
        >
        <RouterLink
          to="/composables/page-refactor/page-after-explained"
          class="active"
          >After Explained</RouterLink
        >
      </nav>
      <h1>After: logic extracted to composables</h1>
      <p class="lede">
        The same four concerns — user, posts, settings, and modal — are each
        handled by a focused composable. Adding a fifth concern means adding one
        more line, not 15.
      </p>
    </header>

    <section class="code-panel code-panel--good" aria-label="Script setup code">
      <div class="code-panel-header">
        <span class="badge badge--good">After</span>
        <span class="code-panel-stat">5 lines in &lt;script setup&gt;</span>
        <span class="code-panel-note">each composable owns one concern</span>
      </div>
...
const { settings, toggleNotifications }                   = useUserSettings(userId)
const { isOpen: modalOpen, open: openModal, close: closeModal } = useModal()</code></pre>
    </section>

    <div class="composables-grid">
      <div
        class="composable-card"
        v-for="item in composableCards"
        :key="item.name"
      >
    ...
    <Teleport to="body">
      ...
      >
        <div class="modal">
          ...
          <p class="modal-note">
            This modal state lives in <code>useModal()</code> — one line to wire
            up <code>isOpen</code>, <code>open</code>, and <code>close</code>.
          </p>
...
<script lang="ts">
const composableCards = [
  {
    name: "useUser(userId)",
    desc: "fetches user, exposes user / loading / error",
  },
  {
    name: "useUserPosts(userId)",
    desc: "fetches posts, re-runs when userId changes",
  },
  {
    name: "useUserSettings(userId)",
    desc: "fetches settings + toggleNotifications()",
  },
  { name: "useModal()", desc: "isOpen / open() / close() / toggle()" },
];
</script>
...
.code-panel--good {
  border-color: rgba(0, 227, 152, 0.4);
  background: rgba(0, 227, 152, 0.03);
}
...
.code-panel-header {
  ...
  border-bottom: 1px solid rgba(0, 227, 152, 0.15);
...
.composable-card {
  ...
  border: 1px solid rgba(0, 227, 152, 0.2);
